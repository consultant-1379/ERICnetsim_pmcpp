...
def check_mandate_param_in_cfg(sim_name):
    ...
    for mo_key in sim_prop_map[sim_name].iterkeys():
...
def get_started_node_info_from_netsim():
    ...
    """
    temp_sim = ""
    for sim_name in sim_prop_map.iterkeys():
...
def map_simulation_with_sim_name():
    ...
    """
    for sim_name in sim_prop_map.iterkeys():
        for simulation_name in started_node_map.iterkeys():
...
def fetch_attribute_from_netsim(sim_name, simulation_name, node):
    ...
    for mo_key, sim_prop_list in sim_prop_map[sim_name].iteritems():
...
def write_transport_file():
    ...
    for simulation_name in writer_map.iterkeys():
        for sftp_key in sftp_order_map.iterkeys():
            if sftp_key in simulation_name:
                ...
                for node in writer_map[simulation_name].iterkeys():
                    node_list = node_list + node + ' '
                file_writer.write(simulation_name.replace('-', '_') + '_node_list="' + node_list[:-1] + '"' + "\n")
                node_list = ""
                for node_name in writer_map[simulation_name].iterkeys():
