...
# Jira No       :  NSS-14486
# Gerrit Link   :
# Description   :  Removing redundant binary files before copying template data to /pms_tmpfs directory.
# Date          :  30/08/2017
...
copyTemplateParts() {
    ...
    for SERVER in $SERVERS ; do
        ...
        if [ ${DEL_TEMPLATE_DIR} -eq 1 ] ; then
            ...
            rsh -l netsim ${SERVER} "rm -rf ${NETSIM_GPEH_TEMPLATE_DIR}"
        fi

        rsh -l netsim ${SERVER} "if [ ! -d ${NETSIM_GPEH_TEMPLATE_DIR} ] ; then mkdir ${NETSIM_GPEH_TEMPLATE_DIR} ; fi"

            # Clear out tmp merge dir
        rsh -l netsim ${SERVER} "rm -rf /tmp/merge"

        rsh -l netsim ${SERVER} "rm -rf /netsim_users/gpeh_template_parts ; mkdir /netsim_users/gpeh_template_parts"
        RNC_LIST=`getSimListForServer ${SERVER}`

        for ROP_PERIOD in ${ROP_PERIOD_LIST} ; do
            log "  ${ROP_PERIOD}min" >> $GENSTATS_CONSOLELOGS

            rsh -l netsim ${SERVER} "mkdir /netsim_users/gpeh_template_parts/${ROP_PERIOD}"

            #ignoring templates other than EBSW for ENM
            if [ ${ENV_TYPE} -eq 0 ] ; then
                rcpFile ${GPEH_TEMPLATE_DIR}/Other_templates.tar.gz  netsim@${SERVER}:/netsim_users/gpeh_template_parts/${ROP_PERIOD}
            ...
            for RNC in ${RNC_LIST} ; do
                ...
                if [ $? -eq 0 ] ; then
                    ...
                    rcpFile ${GPEH_TEMPLATE_DIR}/${RNC}.ebsw_templates.tar.gz  netsim@${SERVER}:/netsim_users/gpeh_template_parts/${ROP_PERIOD}

                    rcpFile ${GPEH_TEMPLATE_DIR}/${ROP_PERIOD}/${RNC}.wncs_templates.tar.gz  netsim@${SERVER}:/netsim_users/gpeh_template_parts/${ROP_PERIOD}
                    rcpFile ${GPEH_TEMPLATE_DIR}/${ROP_PERIOD}/${RNC}.wgeo_templates.tar.gz  netsim@${SERVER}:/netsim_users/gpeh_template_parts/${ROP_PERIOD}
                fi
            done
        ...
        if [ ! -z "${SIM_LIST}" ] ; then
            rcpFile ${GPEH_TEMPLATE_DIR}/rbs_gpeh.bin.gz  netsim@${SERVER}:${NETSIM_GPEH_TEMPLATE_DIR}
        fi
    done
}
...
makeTemplates() {
    ...
    for SERVER in $SERVERS ; do
        ...
        if [ ${ENV_TYPE} -eq 0 ] ; then
            rsh -l netsim ${SERVER} "if [ -d ${DEST_DIR} ] ; then rm -rf ${DEST_DIR} ; fi"
        fi
        fi
        rsh -l netsim ${SERVER} "if [ ! -d ${DEST_DIR} ] ; then mkdir ${DEST_DIR} ; fi"
        ...
        rsh -l netsim ${SERVER} "${MAKE_GPEH_TEMPLATES_CMD} -r \"${MATCHED_RNC_LIST}\" >  ${LOG} 2>&1"
        rsh -l netsim ${SERVER} "tail --lines=100 ${LOG}" | egrep 'ERROR|illegal' > /dev/null
        if [ $? -eq 0 ] ; then
            log "ERROR: ${CMD} failed"
            rsh -l netsim ${SERVER} "tail --lines=100 ${LOG}"
                 exit 1
        fi
        fi
...
moveTemplates(){
    ...
    for SERVER in $SERVERS ; do
        log "  ${SERVER}" >> $GENSTATS_CONSOLELOGS
        rsh -l netsim ${SERVER} "if [ -d ${NETSIM_GPEH_TEMPLATE_DIR} ] ; then rm -rf ${NETSIM_GPEH_TEMPLATE_DIR} ; fi"
        rsh -l netsim ${SERVER} "if [ ! -d ${LINK_GPEH_TEMPLATE_DIR} ] ; then mkdir -p ${LINK_GPEH_TEMPLATE_DIR} ; fi"
        rsh -l netsim ${SERVER} "rm -f ${NETSIM_PMS_DIR}/gpeh_templates/*/*.bin"
        rsh -l netsim ${SERVER} "cp -rf ${NETSIM_PMS_DIR}/gpeh_templates ${LINK_GPEH_TEMPLATE_DIR}/"
    done
}
...
updateCrontab() {
    ...
    for SERVER in $SERVERS ; do
            ...
            rsh -l netsim ${SERVER} "crontab -l | egrep -v '^# |genGPEH|genRbsGpeh' > /tmp/rec_new_crontab"
            ...
            for WORKLOAD in ${GPEH_WORKLOAD_LIST} ; do
                    ...
                    rsh -l netsim ${SERVER} "echo \"${CMD} >> ${NETSIM_LOG_DIR}/genGPEH_${ROP_PERIOD}min.log 2>&1\" >> /tmp/rec_new_crontab"
                    fi
            done
            # RBS GPEH Support
            SIM_LIST=`getRbsGpehSimListForServer ${SERVER}`
            ...
            if [ ! -z "${SIM_LIST}" ] ; then
                ...
                for ROP_PERIOD in ${GPEH_RBS_WORKLOAD};do
                    ...
                    rsh -l netsim ${SERVER} "echo \"${CMD} >> ${NETSIM_LOG_DIR}/genRbsGpeh_${ROP_PERIOD}min.log 2>&1\" >> /tmp/rec_new_crontab"
                done
            
            fi

            rsh -l netsim ${SERVER} "crontab /tmp/rec_new_crontab"
    done
}
...
for SERVER in $SERVERS ; do
    log " $SERVER" >> $GENSTATS_CONSOLELOGS
    rsh -l netsim ${SERVER} "rm -rf /netsim_users/gpeh_template_parts"
    rsh -l netsim ${SERVER} "rm -rf /tmp/merge"
done
