...
cat << EOF
Help:
    Sets tmpfs filesystem for LTE|MGW|SPITFIRE|TCU|RNC|UPG|WCG|WMG|DSC|VRC|VPP|RNNODE|VRM simulations on /pms_tmpfs mount point
...
NODE_TYPES="lte mgw spitfire tcu rnc esapc epg mtas cscf sbg mrf vbgf ipworks hss-fe upg wcg wmg dsc vrc vpp rnnode fronthaul ml dsc dog c608 eme siu02 r6672 r6675 r6274 r6371 r6471-1 r6471-2 vrm"
...
function set_tmpfs() {
    ...
    if [[ ${SIM_NAME} =~ "vRC" || ${SIM_NAME} =~ "vPP" || ${SIM_NAME} =~ "RNN" || ${SIM_NAME} =~ "vSD" || ${SIM_NAME} =~ "5GRadioNode" || ${SIM_NAME} =~ "VTFRadioNode" || ${SIM_NAME} =~ "vRM" ]] ; then
        SIM=$SIM_NAME
        echo "sim is $SIM"
    fi
